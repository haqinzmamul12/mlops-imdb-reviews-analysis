...
class FeatureBuilder:
    ...
    def build_tfidf_features(self):
        try:
            ...
            df_cleaned = pd.concat([df_interim["sentiment"].reset_index(drop=True), X_df], axis=1)
            print(f"TF-IDF features built: {X_df.shape[1]-1} features + sentiment for {X_df.shape[0]} samples")
